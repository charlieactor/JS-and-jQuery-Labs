...
/***************Your Solution****************/
walls.checkCollision = function(obj, direction) {
  if (direction == "left" && obj.xPos <= this.xPos) {
    return true;
  } else if (direction == "right" && (obj.xPos + obj.width) >= (this.xPos + this.width)) {
    return true;
  } else if (direction == "up" && obj.yPos <= this.yPos) {
    return true;
  } else if (direction == "down" && (obj.yPos + obj.height) >= (this.yPos + this.height)) {
    return true;
  } else {
    return false;
  }
...
/***************Your Solution****************/
square.checkForCollisions = function(direction){
  for (var i = 0; i < collidables.length; i++) {
    var bool = collidables[i].checkCollision(this, direction);
    if (bool) {
      return true;
    } else {
      return false;
...
/***************Your Solution****************/
square.moveUp = function(){
  var bool = square.checkForCollisions("up");
  if (bool) {
    return;
  } else {
    this.yPos -= 5;
...
square.moveDown = function(){
  var bool = square.checkForCollisions("down");
  if (bool) {
    return;
  } else {
    this.yPos += 5;
...
square.moveLeft = function(){
  var bool = square.checkForCollisions("left");
  if (bool) {
    return;
  } else {
    this.xPos -= 5;
...
square.moveRight = function(){
  var bool = square.checkForCollisions("right");
  if (bool) {
    return;
  } else {
    this.xPos += 5;
